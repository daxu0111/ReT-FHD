...
class Server(object):
    ...
    def set_clients(self, clientObj):
        for i in range(self.num_clients):
            ...
            #     # model_r = self.add_gaussian_noise(model_r,noise_level = 0.1)
            #     modelname = "noisy"
    ...
    def getter_model(self, modelname, num_classes, dataname):
        ...
        return model
    def add_gaussian_noise(self,model, noise_level):
        with torch.no_grad():  # 不跟踪梯度
            for param in model.parameters():
                # 只对权重添加噪声，不对偏置添加噪声
                if param.requires_grad and len(param.size()) > 1:
                    param.add_(torch.randn(param.size()).to(param.device) * noise_level)
        return model
    ...
    def train_metrics(self):
        ...
        ids = []
        for c in self.clients:
            if c.id not in [21]:  # 排除 ID 为 0 和 1 的客户端
